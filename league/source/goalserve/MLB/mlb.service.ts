...
const get2DaysUpcomingDataFromMongodb = async (date1: string) => {
  let day = moment().format("D");
  let month = moment().format("MM");
  let year = moment().format("YYYY");
  let date = `${day}.${month}.${year}`;
  const date2 = moment(date1).add(24, "hours").utc().toISOString();
  const date3 = moment(date2).add(24, "hours").utc().toISOString();
  try {
    const todayData = await Match.aggregate([
      {
        $addFields: {
          spliteTime: {
            $split: ["$dateTimeUtc", " "],
    ...
    const tomorrowData = await Match.aggregate([
      {
        $addFields: {
          spliteTime: {
            $split: ["$dateTimeUtc", " "],
...
const get20HoursUpcomingGameData = async () => {
  const date1 = moment().utc().toISOString();
  const date2 = moment(date1).add(20, "hours").utc().toISOString();
  try {
...
const getAllUpcomingGameData = async () => {
  try {
    const data = await Match.aggregate([
      {
        $match: {
...
const getAllFinalGameData = async () => {
  try {
    const data = await Match.aggregate([
      {
        $match: {
